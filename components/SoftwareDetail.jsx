    ...
    <>
      {software && <div className="bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8 mx-2">
        ...
        <div className="px-4 lg:px-0">
          ...
          {software.title && <h1 className="titlee mb-8 text-3xl font-semibold text-center"> {software.title}</h1>}
          {software.exerpt && <p className="text-gray-500 text-center my-4">{software.exerpt}</p>}
          {software.slug && <h1 className="text-2xl text-center my-16">{software.desc}</h1>}
          {software.code && <h1 className="text-xl font-semibold mb-4 mx-auto  max-w-xs text-center">Manual Installation</h1>}
          {software.code && <p style={{ fontFamily: "Monaco, monospace", fontSize: "18px", lineHeight: "100%", backgroundColor: "#474747", padding: "0.2em", letterSpacing: "-0.05em", color: "white", textAlign:"center", margin:"12px auto",
    wordBreak: "normal", borderRadius: "5px" }} className='text-red-300  max-w-xs mx-auto'>{software.code}</p>}
          <Ratings id={software._id} />
          <div className="flex justify-center mt-10">

            {software.software_website && <a href={software.software_website} target="_blank" rel="noreferrer">
              <span className="bn632-hover bn27 transition duration-500 ease transform hover:-translate-y-1 inline-block hover:bg-teal-900 bg-teal-600 text-lg font-medium rounded-full text-white px-8 py-3 cursor-pointer mx-auto">Website of the Software</span>
            </a>}
            {software.downloadLink && <a href={dlurl} target="_blank" rel="noreferrer">
              <span className="bn632-hover bn27 transition duration-500 ease transform hover:-translate-y-1 inline-block hover:bg-teal-900 bg-teal-600 text-lg font-medium rounded-full text-white px-8 py-3 cursor-pointer mx-auto">Software Direct Download <DownloadIcon/></span>
            </a>}
          </div>
        </div>
      </div>}
